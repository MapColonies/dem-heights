...
async function registerTestValues(shouldInitTerrainProviders = true): Promise<void> {
  ...
  await (async ()=> {
    ...
    catalogTestRecordsServiceInstance.setValue(Object.fromEntries(
      (demTestCatalogRecords as unknown as PycswDemCatalogRecord[]).map((record) => [record.id as string, record])
    ));
